...
export class MicroLearningCourseFacade {
  ...
  constructor() {
    ...
    // Preview mode server-side. Guest users skip this (API requires auth).
    effect(() => {
      const reel = this.activeReel();
      if (!reel) return;
  ...
  trackActivity(
    ...
    // `myclassactivity` is authenticated — skip the network call for guests.
    // We still persist the local playhead below so the UI keeps progressing
    // within the session; the server-side record only begins after login.
  ...
  handleVideoEnded(): void {
    ...
    if (reel.course_type === 'micro_learning') {
      this.openChapterQuiz();
      return;
    }
