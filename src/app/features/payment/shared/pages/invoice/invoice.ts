...
export class Invoice {
  ...
  readonly userData = computed(() => {
    // if (this.isInvoice()) {
    //   return this.facade.invoiceUserData();
    // }
  ...
  constructor() {
    ...
    // Redirect to home if user logs out while on a payment page
    effect(() => {
    ...
    // Refresh `auth.currentPlan` after a successful payment so the header
    // (and any other consumer of `hasActivePlan`) flips immediately instead
    // of waiting for the next profile fetch. The effect re-runs whenever
    // `paymentStatus()` flips, but the `=== 'success'` guard keeps the
    // refresh to the actual success edge.
    let lastRefreshed: string | null = null;
    effect(() => {
      const status = this.paymentStatus();
      const orderId = this.orderId();
      if (status !== 'success' || !orderId) return;
      // De-dupe: don't re-fetch on every effect re-run for the same order.
      const key = `${orderId}:success`;
      if (lastRefreshed === key) return;
      lastRefreshed = key;
