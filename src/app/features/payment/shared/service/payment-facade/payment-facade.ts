...
import { PROFILE_ROUTES, ProfileFormState } from '../../../../../shared/core/models/profile.model';
import { User } from '../../../../../shared/core/models/auth.model';
...
export class PaymentFacade {
  ...
  async openCouponDialog(): Promise<void> {
    ...
    const { CouponDialog } =
      await import('../../../../../shared/components/dialog/coupon-dialog/coupon-dialog');
  ...
  async openFirmSponsorshipDialog(
    ...
    dialogRef.afterClosed$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((result) => {
      ...
      const existingCompanyId = this.auth.currentUser()?.company?.[0]?.id ?? null;
      ...
      this.http
        ...
        .subscribe((profile) => {
          if (profile?.status && profile.user) {
            this.auth.setAuthenticated(profile.user);
          }
        });
    });
  }
  ...
  async promptPartnerCodeOrSubscribe(
    ...
    // A partner code already applied to this plan, or an active subscription,
    // makes the prompt pointless — skip it and go straight to the normal flow.
    if (plan.is_partner_code_applied || this.auth.hasActivePlan()) {
