 ...
 *   webinar + not Present → View Details (no attendance, no certificate)
 ...
 *
 * The webinar gate mirrors `pickUpcomingAction` and the badge library
 * (`buttonsForCourseItem`): a webinar certificate is earned by attending, so
 * feedback alone must not surface a Download button.
 */
function pickCompletedAction(row: ReportRow): ButtonKind {
  if (row.transaction_type === 'webinar' && row.attendance_status !== 'Present') {
    return 'view-details';
  }
