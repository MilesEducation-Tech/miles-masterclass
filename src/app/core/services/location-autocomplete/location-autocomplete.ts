 ...
 * @param failed    optional flag set when the lookup errors, so a caller can
 *                  fall back to a free-text field instead of leaving the user
 *                  with a picker that can never offer anything. Optional
 *                  out-param rather than a changed return type: the other two
 *                  call sites don't need it and shouldn't have to change.
 */
          ...
          : http
              ...
              .pipe(
                map((res) => {
                  ...
                  failed?.set(false);
                  return (res.data ?? []).map((s) => s.description).filter(Boolean);
                }),
                catchError((err) => {
