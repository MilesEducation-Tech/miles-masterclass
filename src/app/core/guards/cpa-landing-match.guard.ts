...
/**
 * Country segments (ISO2, lower-cased) that should see the CPA marketing
 ...
 */
const CPA_LANDING_COUNTRIES = ['ae'];
const CPA_LANDING_PROFESSION = 'accounting';

/**
 * Decides whether the CPA landing page should handle `home` for the current
 * URL. `country` / `profession_type` are matched by the *parent*
 ...
 */
export const cpaLandingMatchGuard: CanMatchFn = () => {
  ...
  return (
    CPA_LANDING_COUNTRIES.includes((country ?? '').toLowerCase()) &&
    (profession ?? '').toLowerCase() === CPA_LANDING_PROFESSION
  );
};
