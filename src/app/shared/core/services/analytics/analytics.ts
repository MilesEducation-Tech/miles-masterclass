...
export class Analytics {
  ...
  init(): void {
    ...
    afterNextRender(() => {
      ...
      // (Re)identify when the end-user profile or plan resolves/changes.
      // identify() sets GA4 user_id + user properties and Netcore identity +
      // attributes; each call guards its own vendor, so no netcoreLoaded gate here.
      effect(
        () => {
          const user = this.auth.currentUser();
          const plan = this.auth.currentPlan();
          if (user) this.identify(user, plan);
  ...
  flushIdentity(): void {
    const user = this.auth.currentUser();
    if (user) this.identify(user, this.auth.currentPlan());
    else this.resetIdentity();
  }
    ...
    s.onload = () => {
      ...
      if (user)
        this.setNetcoreUser(
          ...
          this.userProperties(user, this.auth.currentPlan()),
        );
      else st('identify', ''); // empty = anonymous
