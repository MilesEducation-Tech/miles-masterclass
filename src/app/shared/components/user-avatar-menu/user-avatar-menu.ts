...
import { Component, PLATFORM_ID, computed, inject, ViewEncapsulation } from '@angular/core';
...
export class UserAvatarMenu {
  ...
  /**
   * Sign out from the avatar dropdown. Hard-navigates to `/` after clearing
   * the session — `clearAuth` alone leaves the user on whatever auth-protected
   * route they were on (the `authGuard` only re-evaluates on the next
   * navigation), and a `router.navigate` here would be blocked by any
   * `canDeactivate` guards on the current route. The full-page reload
   * also flushes route-scoped state from the previous session.
   */
  onLogout(): void {
