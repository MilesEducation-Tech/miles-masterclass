...
export class ProfileCompletionDialog {
  ...
  constructor() {
    // ponytail: an effect here pre-filled sector/job-role from the signed-in
    // user's profile. No profile to read, so the dialog always opens empty.
      ...
      .subscribe({
        next: (res) => {
          ...
          if (res?.status && res.user) {
            // ponytail: also pushed the saved profile into the session service.
