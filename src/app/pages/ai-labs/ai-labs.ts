...
// Type-only: the catalogue no longer opens the agent dialog, it navigates to
// the course page — but the card shape still lives with the dialog.
import type { AiLabAgentCard } from '../../shared/components/dialog/ai-lab-agent-dialog/ai-lab-agent-dialog';
...
import { ContentResponse, TRACK_ROUTES } from '../../shared/core/models/track.model';
...
export class AiLabs {
  ...
  /** The app's own auth, distinct from the Entra session `auth` above. */
  protected readonly milesAuth = inject(Auth);
  ...
  /**
   * Track id → its `ai_lab` courses. `undefined` means still loading; `[]` means
   * the fetch settled with nothing, whether it failed or the track is genuinely
   * empty. The distinction that matters is loading vs settled — a settled empty
   * result is what keeps a section out of the catalogue entirely.
   */
  private readonly trackCourses = signal<Record<number, Content[] | undefined>>({});

  /**
   * The catalogue the template renders — one card per `ai_lab` course, grouped
   * by track.
   *
   * A section only exists here once its track answers with courses: a track
   * that is still loading keeps its heading (so the skeletons have somewhere to
   * render), and one that settles empty — failed, or not published yet — drops
   * out entirely rather than leaving a bare heading over an empty rail.
   */
  protected readonly resolvedSections = computed(() => {
    const courseMap = this.trackCourses();

    return this.sections
      .map((section) => {
        const loadedCourses =
          section.trackId === undefined ? undefined : courseMap[section.trackId];
        const courses = loadedCourses ?? [];
        const cards = courses.map((c): AiLabAgentCard => ({
          name: c.title,
          description: c.course_short_overview,
          // Landscape art matches the card's aspect; `thumbnail` is the
          // portrait crop, used only when the landscape one is missing.
          thumbnail: c.horizontal_thumbnail || c.thumbnail,
        ...
        return {
          ...
          loading: section.trackId !== undefined && loadedCourses === undefined,
          cards,
        };
      })
      .filter((section) => section.loading || section.cards.length > 0);
  });
  ...
  /**
   * Catalogue track rails, every viewport: 1 / 2 / 3 cards per view, matching
   * the grid these rails replaced. `rewind` rather than `loop` so a track with
   * one or two courses needs no clone padding.
   */
   ...
   */
  protected readonly hasLabAccount = computed(
    () => this.milesAuth.currentUser()?.is_ai_lab_user === true,
  );

  /** An account request is in flight (create or resend). */
  protected readonly submitting = signal(false);

  /** A request went out and the credentials email is on its way. */
  ...
  protected readonly stillPending = computed(
    () => this.refreshAttempted() && !this.milesAuth.isLoadingProfile() && !this.hasLabAccount(),
  );
  ...
  protected readonly ctaLabel = computed(() => {
    ...
    // Once the request is out the account is on its way, so the CTA becomes the
    // login the emailed credentials are for — not a second create.
    if (this.milesAuth.isLoggedIn() && !this.hasLabAccount() && !this.accountRequested())
      return 'Create AI Lab account';
  ...
  private loadCatalogue(): void {
    for (const section of this.sections) {
      if (section.trackId !== undefined) this.loadTrackCourses(section.trackId);
    }
  }

  /**
   * A track's `ai_lab` courses — the catalogue source that outranks the
   * static agents below. Silent on failure, settling to `[]` so the section
   * falls through to its fallback.
   *
   * ponytail: first page only. Wire the rail's `(loadMore)` to `?page=N` when
   * a track outgrows one page.
   */
  private loadTrackCourses(trackId: number): void {
    this.apiClient
      .get<ContentResponse>(TRACK_ROUTES.trackContent.path.replace(':id', String(trackId)), {
        params: { course_type: 'ai_lab' },
        context: new HttpContext().set(SKIP_ERROR_NOTIFICATION, true),
      })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (res) => this.setTrackCourses(trackId, res?.data ?? []),
        error: () => this.setTrackCourses(trackId, []),
      });
  }

  private setTrackCourses(trackId: number, list: Content[]): void {
    this.trackCourses.update((map) => ({ ...map, [trackId]: list }));
  }

  /** Card click — every tile is a course now, so every tile has a page. */
  protected viewAgent(card: AiLabAgentCard): void {
    if (card.courseId === undefined) return;
    this.utils.navigateToCourse('ai-labs', card.courseId, card.name);
  }
  ...
  protected activate(): void {
    if (!this.milesAuth.isLoggedIn()) {
      // `redirect`, not `returnUrl` — that's the param the login flow reads back.
      this.gatePrompt(
        'Login Required',
        'Sign in to your Miles account to access Miles AI Labs.',
        'Login',
        () =>
    ...
    // Same fallback as `activePlanGuard`: on a hard refresh `currentPlan` is
    // null until the async plan fetch lands, so an active subscriber would be
    // wrongly gated without the cookie snapshot.
    if (!this.milesAuth.hasActivePlan() && !this.milesAuth.hasActivePlanFromCookie()) {
      this.gatePrompt(
        'Subscription Required',
        'An active plan is required to access Miles AI Labs. Subscribe to unlock the agent catalogue.',
        'View Plans',
        () =>
    ...
    // Third gate: there is no Microsoft account to sign in to yet. Sits here
    // rather than in the template so a card's Launch button can't skip it.
    if (!this.hasLabAccount() && !this.accountRequested()) {
      this.openTerms();
      return;
    }
      ...
      .subscribe({
        next: () => {
          ...
          this.notification.success(
            'Account requested',
            "We're creating your AI Lab account. Check your email in a minute for your sign-in details.",
          );
        },
   ...
   * AI Lab password and emails the new credentials. Only reached through
   * `confirmResend`.
   */
  private resendEmail(): void {
    ...
    this.apiClient
      ...
      .subscribe({
        next: () => {
          this.submitting.set(false);
          this.notification.success('Email sent', 'Check your inbox for your new AI Lab password.');
        },
        error: () => this.submitting.set(false),
      });
  }
