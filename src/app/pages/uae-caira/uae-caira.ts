...
export class UaeCaira {
  ...
  /**
   * ponytail: inert. Was the live/next-up webinar from `WebinarFacade`, which
   * went with the webinar data layer — the hero registration block now renders
   * its empty-state design. Re-wire by restoring the facade and pointing this
   * at `liveOrNextUp`.
   */
  protected readonly heroWebinar = signal<UpcomingPremiere | null>(null);
  protected readonly heroSession = computed(() => this.heroWebinar()?.webinar_dates?.[0] ?? null);
