...
import { logo, mcGrawHillLogo } from '@core/constant/icon';
import { User } from '@core/models/profile.model';
import { CountryCodeOption } from '@core/constant/dial-code';
...
/**
 * Response from the faculty OTP-verify endpoint. Lived in `auth.model.ts`
 * until the auth layer was removed; faculty is its only consumer.
 */
...
export class Faculty {
   ...
   * ponytail: was `auth.hasActivePlan()`. Nothing locks the form now.
   */
  protected readonly locked = signal(false);

  /**
   * Map the authenticated user onto the form shape.
   * ponytail: nothing supplies a user any more, so this always maps `null` —
   * i.e. the form starts empty.
   */
  ...
  protected readonly model = linkedSignal<FacultyFormState>(() => this.mapUserToForm(null));
   ...
   */
  // ponytail: compared the typed email against the signed-in account's. With
  // no account to compare to, every submission verifies — which is the side
  // that can't hand out access to an unproven address.
  protected readonly otpRequired = computed(() => true);
  ...
  protected readonly facultyForm = form<FacultyFormState>(this.model, (s) => {
    ...
    // ponytail: each of these also froze a field the profile already knew
    // (`auth.currentUser()?.first_name` and friends). Only the lock is left.
    disabled(s.first_name, { when: () => this.locked() });
    disabled(s.last_name, { when: () => this.locked() });
    disabled(s.country_code, { when: () => this.locked() });
    disabled(s.phone, { when: () => this.locked() });
    disabled(s.location, { when: () => this.locked() });
  ...
  private handleRegisterResult(
    ...
    // ponytail: refreshed the cached user here so the header reflected what
    // the backend had just saved.
  ...
  /**
   * Post-verification analytics.
   *
   * ponytail: was the auto-login — it mirrored `AuthFacade.verifyOtp` so both
   * entry points left identical auth state behind. The token pair is still
   * passed in (the endpoint returns it) but nothing consumes it any more.
   */
  private completeAutoLogin(
    ...
    // ponytail: stored the token pair and opened the session here. Without an
    // auth layer the OTP now only proves the address — it logs nobody in, and
    // the analytics below is all that is left of this path.
