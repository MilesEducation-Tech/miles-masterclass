...
- Build identity flows through `scripts/generate-version.mjs`, which writes **both**
  `src/app/core/version/app-version.ts` (baked into the browser and server bundles) and
  `public/version.json` (`{version, sha, builtAt, buildId}`, served at `/version.json`). **There is no
  `APP_BUILD` define — use `APP_VERSION`**, which is the `sha.timestamp` build id, so a redeploy of the
  same commit still trips the update prompt. Both outputs are gitignored and regenerated by the `pre*`
  hooks. Never `import` `package.json` into app code.
