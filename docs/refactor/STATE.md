...
- 🟡 **PHASE 10 (headless UI) — row `core/services`: the Dialog service → ng-primitives, 2026-09-25.**
  ...
  **Target pattern (every dialog):**
  ...
  - [ ] B2b — offerings/tracker/library: `AssessmentResultDialog`, `CairaBadgeInfoDialog`, `CourseFiltersDrawer` (drawer), `CpeComplianceDialog`, `HtmlContentDialog`, `MicroLearningAboutPanel` (drawer; hosts `course-about` → its dual `closeAll` can drop the old manager after this), `MicroLearningFilterSheet`, `MicroLearningQuizDialog`, `SelectCpeMode`. - [ ] B3 — `admin/*` dialogs.
  - [ ] B4 — `UtilsDialog` (18 calls) + `closeAll`/`afterOpened$` callers; delete `Dialog`, its CSS and
        ...
        **Next:** commit B2a, then `/refactor-phase 10 core/services` resumes at B2b.
