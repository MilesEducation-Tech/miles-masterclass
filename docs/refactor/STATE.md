...
- 🟡 **PHASE 10 (headless UI) — row `core/services`: the Dialog service → ng-primitives, 2026-09-25.**
  ...
  **Target pattern (every dialog):**
  ...
  - [ ] B4 — `UtilsDialog` (18 calls) + `closeAll`/`afterOpened$` callers; delete `Dialog`, its CSS and
        ...
        **Next:** commit B3, then `/refactor-phase 10 core/services` runs B4 — `UtilsDialog` (18 calls in 16 files), then delete `core/services/dialog/dialog.ts` + spec, `src/styles/dialog.css` (+ its `angular.json` entries), `video-poster`'s dual listener, and the old-API mentions in stories/mocks/`feature-dialog-tokens.ts`; close the row ✅.
