...
- Phase: **5 — Features 🟡.** Session **7 `payment` ✅ COMPLETE** (2026-09-23).
  `verifier` **8/8 green**, `reviewer` **PASS, zero violations**. Report:
  [phase-05-payment](reports/phase-05-payment.md). 62 renames, 17 files with a content diff, every
  hunk a single import-specifier swap.
- 🏁 **ONE SESSION LEFT IN PHASE 5: `offerings`** — 5 layers, 106 files, and it must be run as **one
  session**, because its sub-features reach up into `offerings/shared/` with `../../../../shared/…`
  and splitting rewrites those ~92 lines twice with a broken window in between.
- **The invariance rule held twice more, then broke once — and the break was predictable.**
  `../../components/…` was untouched, and so was `plan-comparison-table.ts`'s
  `../../constants/plan-icons`, because `shared/constants/` and `shared/components/` dropped the
  layer together. **`service/` → `services/` collapsed TWO segments** (singular→plural AND
  folder-per-service→flat file), so all 17 crossers changed. **The test is whether both endpoints
  move by the same amount, not whether a `shared/` folder is involved.**
- ⚠️ **All 17 `PaymentFacade` importers were repointed, and NOT ONE boundary violation was fixed.**
  Five of them are §3 violations and are exactly as illegal at `@features/payment/services/…` as they
  were before. **Shorter import paths are not progress on boundaries.** The census is unchanged at
  **9 edges**, six of them `PaymentFacade`. Phase 7 still owns all nine.
- Bundle **+0.0%**, lazy **271 unchanged** — which mattered here, since `PaymentFacade` is reached
  from `shared/`, `layout/`, `offerings/` and its own feature.
- ⚠️ **No payment route is in the SSR smoke set.** QA list in the report §4 walks the whole checkout
  flow; the `overview-wrapper` shell is the one to watch, since its three children go blank together
  rather than erroring individually if it fails to resolve.
- 🚨 **SEVEN units uncommitted**, and `public/version.json` + `core/version/app-version.ts` are dirty
  from build-timestamp regeneration in every session (open question 6) — exclude them when committing.
- **Next:** `/refactor-phase 5 offerings` — the last one.
