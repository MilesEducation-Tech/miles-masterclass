...
- 🟡 **PHASE 10 (headless UI) — row `core/services`: the Dialog service → ng-primitives, 2026-09-25.**
  ...
  **Target pattern (every dialog):**
  ...
  - [ ] B2 — `features/*` dialogs. - [ ] B3 — `admin/*` dialogs.
  - [ ] B4 — `UtilsDialog` (18 calls) + `closeAll`/`afterOpened$` callers; delete `Dialog`, its CSS and
        ...
        **Next:** commit B1b, then `/refactor-phase 10 core/services` resumes at B2 (`features/*` dialogs — `SubscriptionDialog` with its cart drawer / partner-code prompt / firm-sponsorship children as one set; also `engagement-dialog`'s `afterClosedOld` and `course-about`'s dual `closeAll` can shrink once their last old dialogs move). ⚠️ Migrate nested pairs together —
